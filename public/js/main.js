	...
	$.cecp("cecplugin.page", {
		...
		coverHtml : function(o){
			var html = [];
				...
				//修改分页结构及样式
				html.push('<div class="page private-pager"><div class="p-wrap"><span class="p-num fl"><a class="pn-prev" name="page-left"><</a>');
				html.push('<div class="div-num private-page-list"></div><a href="javascript:void(0);" class="pn-next" name="page-right">></a></span>');
		...
		_dealDom : function(){
			...
			if(o.pageNo<2){
				$(cover).find("[name=page-left]").addClass("disabled");
			}else{
				if(o.bindtype=="ajax"){
			...
			if(o.pageNo>=o.totalPages){
				$(cover).find("[name=page-right]").addClass("disabled");
 ...
 * 
 * $.getRecommend({
 * 		tableCode:109007,//表编码
 *		opeType:3010900701001,//图片码
 *		pageSize:5,//显示多少数据
 *		sort:"view_num",//补足数据排序字段
 *		order:"desc"//补足数据排序方式
 *   });
 *   
 *   列表显示位置的id为recommendList
 *  <idv id="recommendList" />
 *   
 */
jQuery.getRecommend = function(data){
	$.ajax({
		type : "post",
		url : rootPath + "/ajax/data/recommend/list",
		dataType : 'json',
		data : data,
		cache : false,
		success : function(json) {
			if (json.result == 1) {
				recommendList.getRecommendList(json);
			} else {
				//alert(json.mes);
			}
		}
...
var recommendList = {
	getRecommendList:function(json){
		var model = json.model;
		for(var i=0,len=model.rec.length;i<len;i++){
			var html = recommendList.Html(model.rec[i]);
			$("#recommendList").append(html);
		}
	},
	Html:function(rec){
		var imgPath = imagePath + "/" + rec.logo;
		var html = '<a href="'+rootPath+'/asp/needs/detail/'+rec.id+'"><img src="'+imgPath+'" width="288px" height="150px" style="cursor:pointer;/></a>';
			html += '<a href="'+rootPath+'/asp/needs/detail/'+rec.id+'"><span>'+rec.title+'</span></a>';
		return html;
	}
};
